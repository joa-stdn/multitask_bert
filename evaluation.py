...
def model_eval_multitask(sentiment_dataloader,
    ...
    with torch.no_grad():
        ...
        # Evaluate cfimdb classification.
        cfimdb_y_true = []
        cfimdb_y_pred = []
        cfimdb_sent_ids = []

        for step, batch in enumerate(tqdm(cfimdb_dataloader, desc=f'eval', disable=TQDM_DISABLE)):
            b_ids, b_mask, b_labels, b_sent_ids = batch['token_ids'], batch['attention_mask'], batch['labels'], batch['sent_ids']
            ...
            cfimdb_y_pred.extend(y_hat)
            cfimdb_y_true.extend(b_labels)
            cfimdb_sent_ids.extend(b_sent_ids)

        cfimdb_accuracy = np.mean(np.array(cfimdb_y_pred) == np.array(cfimdb_y_true))

        # Evaluate yelp classification.
        yelp_y_true = []
        yelp_y_pred = []
        yelp_sent_ids = []

        for step, batch in enumerate(tqdm(yelp_dataloader, desc=f'eval', disable=TQDM_DISABLE)):
            b_ids, b_mask, b_labels, b_sent_ids = batch['token_ids'], batch['attention_mask'], batch['labels'], batch['sent_ids']
            ...
            yelp_y_pred.extend(y_hat)
            yelp_y_true.extend(b_labels)
            yelp_sent_ids.extend(b_sent_ids)
        
        yelp_accuracy = np.mean(np.array(yelp_y_pred) == np.array(yelp_y_true))
        ...
        print(f'CFIMDB classification accuracy: {cfimdb_accuracy:.3f}')
        print(f'Yelp classification accuracy: {yelp_accuracy:.3f}')

        return (paraphrase_accuracy, para_y_pred, para_sent_ids,
                sentiment_accuracy,sst_y_pred, sst_sent_ids,
                sts_corr, sts_y_pred, sts_sent_ids, cfimdb_accuracy, cfimdb_y_pred, cfimdb_sent_ids, yelp_accuracy, yelp_y_pred, yelp_sent_ids)
...
def test_model_multitask(args, model, device):
        sst_test_data, num_labels,para_test_data, sts_test_data, cfimdb_test_data, _, yelp_test_data = \
            load_multitask_data(args.sst_test,args.para_test, args.sts_test, args.cfimdb_test, \
            args.yelp_test, split='test')

        sst_dev_data, num_labels,para_dev_data, sts_dev_data, cfimdb_dev_data, _, yelp_dev_data = \
            load_multitask_data(args.sst_dev,args.para_dev,args.sts_dev, args.cfimdb_dev,\
            args.yelp_dev,split='dev')
        ...
        cfimdb_test_data = SentenceClassificationTestDataset(cfimdb_test_data, args)
        ...
        yelp_test_data = SentenceClassificationTestDataset(yelp_test_data, args)
        yelp_dev_data = SentenceClassificationDataset(yelp_dev_data, args)

        yelp_test_dataloader = DataLoader(yelp_test_data, shuffle=True, batch_size=args.batch_size,
                                            collate_fn=yelp_test_data.collate_fn)
        ...
        dev_paraphrase_accuracy, dev_para_y_pred, dev_para_sent_ids, \
            ...
            dev_sts_y_pred, dev_sts_sent_ids, dev_cfimdb_accuracy, dev_cfimdb_y_pred,\
                  dev_cfimdb_sent_ids, dev_yelp_accuracy, dev_yelp_y_pred, dev_yelp_sent_ids = model_eval_multitask(sst_dev_dataloader,
                                                                    para_dev_dataloader,
                                                                    sts_dev_dataloader, cfimdb_dev_dataloader,yelp_dev_dataloader, model, device)
        ...
        with open(args.cfimdb_dev_out, "w+") as f:
            print(f"dev cfimdb acc :: {dev_cfimdb_accuracy :.3f}")
            f.write(f"id \t Predicted_Sentiment \n")
            for p, s in zip(dev_cfimdb_sent_ids, dev_cfimdb_y_pred):
                f.write(f"{p} , {s} \n")

        with open(args.yelp_dev_out, "w+") as f:
            print(f"dev yelp acc :: {dev_yelp_accuracy :.3f}")
            f.write(f"id \t Predicted_Sentiment \n")
            for p, s in zip(dev_yelp_sent_ids, dev_yelp_y_pred):
                f.write(f"{p} , {s} \n")
