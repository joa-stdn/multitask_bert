...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    S = torch.matmul(query, key.transpose(-1, -2)) / (self.attention_head_size ** 0.5)
    ...
    # (bs, num_attention_heads, seq_len, seq_len) x (bs, num_attention_heads, seq_len, hidden_size) -> (bs, num_attention_heads, seq_len, hidden_size)
    y = torch.matmul(S, value)

    # Concatenating multi-heads
